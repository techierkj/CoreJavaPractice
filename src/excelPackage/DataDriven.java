...
public class DataDriven {
	...
	public ArrayList<String> getData(String testcaseName) throws IOException {
		...
		FileInputStream fis = new FileInputStream("C:\\CPQ_Test_Data.xlsx");
		XSSFWorkbook workbook = new XSSFWorkbook(fis);

		int sheets = workbook.getNumberOfSheets();
		for (int i = 0; i < sheets; i++) {
			if (workbook.getSheetName(i).equalsIgnoreCase("SITCredentials")) {
				...
				while (ce.hasNext()) {
					Cell value = ce.next();
					if (value.getStringCellValue().equalsIgnoreCase("uniqueID")) {
						coloumn = k;
					}
					k++;
				}
